...
const DetailCard = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const response = await fetch("https://e2c7-196-117-24-244.ngrok-free.app/Argan_beauty/cart.php", {
        method: "POST",
  ...
  useEffect(() => {
    ...
    fetch(`https://e2c7-196-117-24-244.ngrok-free.app/Argan_beauty/api.php?id=${id}`, {
      method: 'GET',
      headers: {
    ...
    <div className="container mx-auto px-4 py-8 mt-16">
      ...
      <div className="grid md:grid-cols-2 gap-10 bg-white shadow-lg rounded-xl overflow-hidden mt-14">
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="flex justify-between items-center">
            <span className="text-3xl font-bold text-green-600">
              {product.price ?` ${product.price*quantity} $ `: 'Prix sur demande'}
            </span>
        ...
        {isMessageVisible && error && (
          <div className={`fixed bottom-16 left-1/2 transform -translate-x-1/2 p-4 rounded bg-red-200 text-red-400 max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl w-auto 
                ${isMessageVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}
                transition-all duration-300 ease-in-out`}>
          <h2 className="text-center text-sm sm:text-base md:text-lg">{error}</h2>
          </div>
        )}
        { message && (
          <div
          className={`fixed bottom-16 left-1/2 transform -translate-x-1/2 p-4 rounded bg-green-200 text-green-400 max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl w-auto
            ${isMessageVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}
            transition-all duration-300 ease-in-out`}
        >
          <h2 className="text-center text-sm sm:text-base md:text-lg">{message}</h2>
        </div>
