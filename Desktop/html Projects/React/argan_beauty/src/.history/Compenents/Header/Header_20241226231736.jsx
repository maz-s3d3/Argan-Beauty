...
function ProHeader() {
  ...
  onst [cartCount, setCartCount] = useState(0);
  ...
  useEffect(() => {
      ...
      setIdentity(storedIdentity)},[identity,id_cart]);
  ...
  const handleScroll = useCallback(() => {
   ...
   const updateCart=()=>{
    const cartItem=JSON.parse(localStorage.getItem('quantity'))||[];
    let total=cartItem.reduce((sum,item)=>sum+(item.quantity||1),0)
    setCardCount(total);
   }
   useEffect(()=>{
    updateCart();
