...
const Cart = ({ removeFromCart, isDarkMode }) => {
  ...
  const handleDelete = (id_cart_item) => {
    setIsRemoving(id_cart_item);
    axios
      .delete(`https://2f98-196-117-24-244.ngrok-free.app/Argan_beauty/cart.php?id_cart_item=${id_cart_item}`)
      .then(() => {
        setItems((prevItems) => prevItems.filter((item) => item.id_cart_item !== id_cart_item));
        setIsRemoving(null);
        const toastElement = document.createElement('div');
        toastElement.className = 'toast-success';
        toastElement.innerText = 'Article supprimé avec succès';
