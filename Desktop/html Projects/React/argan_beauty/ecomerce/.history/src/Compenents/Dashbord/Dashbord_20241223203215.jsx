...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch("https://fe14-196-117-24-244.ngrok-free.app/Argan_beauty/users.php");
  ...
  const handleDelete = async (userId) => {
    if (window.confirm("Are you sure you want to delete this user?")) {
      try {
        const response = await fetch(`https://fe14-196-117-24-244.ngrok-free.app/Argan_beauty/users.php/${userId}`, {
          method: "DELETE",
        });
        if (!response.ok) {
