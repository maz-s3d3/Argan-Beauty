...
const useCart = (id_cart) => {
  ...
  const addToCart = async (productId) => {
    try {
      const response = await fetch(
        'https://2f98-196-117-24-244.ngrok-free.app/Argan_beauty/add_to_cart.php',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({ id_cart, productId }),
        }
      );
  ...
  return { cartCount, updateCartCount, addToCart };
};
...
function ProHeader() {
  ...
  const { cartCount, updateCartCount } = useCart(id_cart);
